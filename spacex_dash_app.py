...
df = spacex_df
print(df.describe())
siteList = df['Launch Site'].unique()
print(siteList)
xdata = df.groupby('Launch Site')
print(xdata.size())
xs = df.groupby('Launch Site')['class'].sum()
xs = xs / xs.sum()
print('[0.a]success rates = \n', xs)
print('\n')
...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider',
                                                ...
                                                marks={0: '0', 25: '25k', 50: '50k', 7500: '75k', 10000: '100k'},
...
def get_pie_chart(entered_site):
    print('\n[1] selected site = ', entered_site)
    filtered_df = spacex_df
    if entered_site == 'ALL':
        xs = filtered_df.groupby('Launch Site')['class'].sum()
        xs = xs / xs.sum()
        print('\n[2] success rates = ', xs)
        fig = px.pie(xs, values='class',
        names = 'class', 
        ...
        xs = df.groupby('class').count()
        xs = xs / xs.sum()
        print('\n[3] selected site data')
        print(df)
        print('\n[4] site rate = ', xs)
...
def get_scatter_chart(entered_site, slider_value):
    ...
    if entered_site == 'ALL':
        ...
        df = df.loc[df['Launch Site'] == entered_site]
        print('\nslider value = ')
        print(slider_value)
        f = (df ['Payload Mass (kg)'].between(slider_value[0], slider_value[1]))
        df = df.loc[f]
        print(df)
        fig = px.scatter(df, x = 'Payload Mass (kg)', y = 'class', color = 'Booster Version Category',
